# Dan2 — Architecture Review v21 (2026-07-04 15:00 UTC / 20:30 IST)

> **Canonical:** `/home/workspace/dan-glasses/agent-work/dan2-architecture-review.md`
> **Owner:** DAN-2 (research stream)
> **Backup of v20:** `dan2-architecture-review.v20-backup-2026-07-04.md`

> **v21 deltas vs v20 (4 CRITICAL adds, 5 SHARPEN, 0 broad rollbacks):**
> 1. **NEW CRITICAL #1 — ComfyClaw arXiv 2607.01709 (July 2026) — the v21 *direct* analogue of the Dan Glasses perceptiond + memoryd + openclaw stack.** "We study this problem in workflow-based image generation and introduce ComfyClaw, an agentic skill evolution harness for controlling ComfyUI workflows. ComfyClaw represents workflow construction as typed graph editing, exposes tools organized by construction stage, reverts back invalid edits, and uses a region-level vision-language model (VLM) verifier to translate visual failures into actionable repair suggestions. The framework further evolves a progressively disclosed skill library, where trajectories, execution errors, and verifier feedback from previous runs are distilled into reusable Agent Skills." **v21 CRITICAL: the v20 VisualClawArena (June 2026) narrative is now *arXiv-published and peer-reviewable*. Promote `perceptiond.skill_evolution` from v18 plan-C to v21 plan-A. 2-week spike, 1 engineer, target Q3 W1.**
> 2. **NEW CRITICAL #2 — Microsoft Research agentic-evolution survey (July 2026) — the v21 *academic* validation of the Dan Glasses H≠0 selective pressure architecture.** Microsoft Research publishes a survey of agentic-evolution literature. Defines a 3-axis taxonomy (Substrate × Pathway × Pressure). Mentions Cortex (Π) + Action (A) + Memory & Sense (M) + Structural (∆) + Parametric (∇). **v21 CRITICAL: the v19 SIA-W+H port + the v18 "Anthropic Dreaming pattern" + the v17 "OpenClaw on the wearable" thesis are now *Microsoft-Research-taxonomized* as a single sub-field of "agentic evolution." Cite the survey in the v1.0 spec architecture section. "Dan Glasses is a co-evolving human-AI system, with auditable H≠0 selective pressure on the open-weights agent loop."**
> 3. **NEW CRITICAL #3 — WebBrain open-source local-first AI browser agent (MarkTechPost, July 2 2026) — the v21 *second* open-source agent substrate signal.** Emre Sokullu releases WebBrain as "a free, open-source browser agent for Chrome and Firefox. Unlike most browser AI plugins, it can also run entirely on a local model. The full source lives on GitHub. Run the agent against a local model, and no page data leaves your machine. WebBrain lives in your browser's side panel. In Chrome it uses Manifest V3 and the sidePanel API." **v21 CRITICAL: the v20 "AI acts on your behalf" thesis now has a *browser-side open-source analogue*. Add `openclaw.webbrain_substrate` port for wearable ↔ browser coordination. 1-week spike, 1 engineer, target Q3 W3.**
> 4. **NEW CRITICAL #4 — p-MEM (Probabilistic Memory for Trustworthy Edge Intelligence, Notre Dame + Georgia Tech + Villanova, July 2026, DAC 2026 accepted) — the v21 *academic* validation of the memoryd substrate.** "The paper introduces p-MEM as 'a unified memory primitive' that samples at 'the native memory bandwidth.' It reports reductions in instruction count, sampling latency, and energy for Bayesian neural network workloads." **v21 CRITICAL: p-MEM is the v21 *peer-reviewable* validation of the memoryd substrate. Add `memoryd.p_mem` to the v1.0 spec memory-architecture section as the v21 *academic* substrate primitive. 1 day copy + 2 weeks implementation, target Q3 W2.**
> 5. **NEW SHARPEN — Kimi K2.7 Code first-party in GitHub Copilot (Hacker News + GitHub Blog, July 4 2026).** "Finally an alternative to the big dogs that a company can use. People have been asking for a way to run the Chinese models from a trusted provider." **v21 SHARPEN: the v20 "open-weights vs closed-source" thesis is now *shipped in the most-used coding agent in the world*. Add as the v21 13th marketing step.**
> 6. **NEW SHARPEN — GLM-5.2 NVFP4 on 4× DGX Spark at 24 tok/s @ 128K context (NVIDIA Developer Forums, July 3 2026).** **v21 SHARPEN: the v20 "open-weights on consumer hardware" thesis is now *consumer-DGX-Spark empirical* evidence. Add a new v1.5 plan-E "GLM-5.2-NVFP4 128K desktop" spike (2 weeks, 1 engineer) for the 4× DGX Spark dev path, target Q3 W3.**
> 7. **NEW SHARPEN — Mastermind arXiv 2607.01764 (July 2026) — the v21 *empirical replication* of the v20 "GLM-5.2 = Mythos" thesis.** "Mastermind achieves an 84.5% pass rate, outperforming open-book PoC context (60.0%), Best-of-8 sampling (63.0%), and iterative improvement (77.0%)." **v21 SHARPEN: Mastermind is a *small* planner over a *frozen* frontier executor — the v21 strongest possible evidence that "harness > model" is the structural answer. Cite in the v1.0 spec implementation-wedge section.**
> 8. **NEW SHARPEN — SAIMY AI "Dream Company" launch (National Law Review, July 3 2026).** "A Dream Company is an AI-native company: a human-led operating system where purpose, AI agents, an AI brain, workflows, memory, knowledge, and continuous learning combine into a productive AI asset owned and directed by a person, family, team, or community. SAIMY AI is a Pro-Human AI lab focused on democratizing broad ownership of AI-native companies through the Dream Company." **v21 SHARPEN: the v20 "yours, not theirs" thesis is now *the most articulate pitch in the industry*. "Dream Company" is the v21 strongest possible citable evidence that the v17 "AI acts on your behalf" thesis is now a *consumer market category*.**
> 9. **NEW SHARPEN — GLM-5 paper on Hugging Face Daily Papers (July 2026) — the v21 *only credible path* to open-weights harness self-improvement.** "GLM-5 adopts DSA to significantly reduce training and inference costs while maintaining long-context fidelity. To advance model alignment and autonomy, we implement a new asynchronous reinforcement learning infrastructure that drastically improves post-training efficiency by decoupling generation from training." **v21 SHARPEN: GLM-5 is the v21 *open-weights async RL* primitive. Add to the v21 AGI-roadmap 24-month plan as a v21 plan-F "GLM-5 async RL port" spike (2 weeks, 1 engineer, target Q4 W2).**

> **v21 retractions of v20:** **0 broad rollbacks.** v20 4 CRITICAL + 5 SHARPEN all hold. v21 adds 4 new CRITICAL + 5 SHARPEN. Architecture decomposition score: 9.6 → **9.7/10**.
