# Dan Glasses Model Selection — Deep Dive

**Author:** Dan-2
**Date:** 2026-07-01
**Scope:** LFM2.5-VL-450M, whisper.cpp, KittenTTS — are these still right? What alternatives exist?
**Status:** Confirmation + v1.5 candidate list

---

## TL;DR

**v1 (current): LFM2.5-VL-450M ✅, whisper.cpp + base.en ✅, KittenTTS ✅**
**v1.5 candidates: SPEED-Q 4-bit, Phi-4-Multimodal (text+vision+audio unified), Kokoro-82M q4**
**v2 candidates: MiniCPM-V 2.5 (8B), Moshi-style native multimodal**
...
4. **Verified live** in the perceptiond daemon. event_id 280+ in Dan-3's notes. ~10-15s/frame on x86_64 CPU, target sub-250ms is reachable with proper NPU/GPU.
5. **Already shipped by the team.** The work is done.
...
|---|---|---|---|---|
| **LFM2.5-VL-450M (current)** | 389MB | SOTA small VLM, multilingual, bbox, sub-250ms | Liquid benchmarks only, unverified power | **KEEP for v1** |
...
| **Phi-4-Multimodal (5.6B)** | ~2.4GB Q4 | Speech + vision + text unified, 128K context | 6× bigger, slower | **v2 (one model instead of 3)** |
...
- **v2 (Q2 2027):** Evaluate Phi-4-Multimodal as a v1 → v2 unification. Replaces LFM2.5 + whisper.cpp with a single 5.6B model. Trade: 6× bigger, 6× slower, but one model to manage.
...
4. **Verified live** in the audiod daemon. 150/150 tests passing per dan2's notes. v1.1 liveness/readiness split is the K8s-grade contract.
...
| Model | Size | Strengths | Weaknesses | Verdict |
|---|---|---|---|---|
| **whisper.cpp base.en (current)** | 74MB | Production, multilingual, GPU, VAD | English-only | **KEEP for v1** |
...
- **Multilingual (Q2 2027 if needed):** Switch to `base` (multilingual) for Hindi, Spanish, etc. 74MB, slightly worse English.
...
4. **Verified live** in the ttsd daemon. 6/6 tests passing. Sub-1s warm-path latency per the spec.
...
|---|---|---|---|---|---|
| **KittenTTS medium (current)** | ~25MB | 8 | Good | Medium | **KEEP for v1 (size matters)** |
| **Kokoro-82M q4** | ~90MB | 54, 9 langs | SOTA local | Fast | **v1.5 candidate (quality matters more)** |
...
- **v1.5 (Q4 2026):** A/B test KittenTTS vs Kokoro-82M q4 vs Piper int8 on a real user study. Default to whichever wins on the (quality + size + speed) Pareto frontier. **Hypothesis:** Kokoro q4 will win on quality at acceptable size.
- **v2 (Q2 2027):** If Kokoro q4 is adopted, ship with multilingual (Hindi, Tamil, etc.) for India market. KittenTTS stays as a size-constrained fallback.
...
4. **Verified live** in the memoryd daemon. 16/16 tests passing. DB at ~300KB after 4 memories.
...
|---|---|---|---|
| **all-MiniLM-L6-v2 (current)** | 384-dim, fast, well-known | Flat cosine, no temporal | **KEEP for v1** |
...
| **OpenAI text-embedding-3-small (cloud)** | 1536-dim, top quality | Cloud dependency | **NO (violates local-first)** |
...
|---|---|---|---|
| **Vision** | LFM2.5-VL-450M Q4_0 | LFM2.5-VL-450M SPEED-Q 4-bit + INT8 mmproj | LFM2.5-VL-1.6B or Phi-4-Multimodal |
| **STT** | whisper.cpp base.en | + openWakeWord, evaluate Parakeet/Moonshine | Phi-4-Multimodal native ASR (if unified) |
| **TTS** | KittenTTS medium | A/B test Kokoro-82M q4, Piper int8 | Kokoro-82M q4 multilingual |
| **Memory embeddings** | all-MiniLM-L6-v2 flat cosine | True Memory (encoding-gate) + Matryoshka | Engram or Memanto (if needed) |
