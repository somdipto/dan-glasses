# Dan2 — Model Analysis v28 (2026-07-05 10:30 UTC / 16:00 IST)

> **Canonical:** `/home/workspace/dan-glasses/agent-work/dan2-model-analysis.md`
> **Owner:** DAN-2 (research stream)
> **Backup of v27:** `dan2-model-analysis.v27-backup-2026-07-05.md` (preserved verbatim, mtime 2026-07-05 04:00 UTC)
> **Status:** v28 SHIPPED. v27 content preserved verbatim below.

> **v28 deltas vs v27 (0 model swaps, 1 NEW SHARPEN, 1 NEW HONORABLE MENTION, 0 retractions):**
> 1. **NEW SHARPEN #1 — LFM2.5-VL-450M confirmed best-fit VLM for v1.0 (held from v17).** LFM2.5-VL-450M (Liquid AI, April 11 2026) is the v28 still-only sub-500MB VLM with native llama.cpp GGUF + ONNX + a SigLIP2 NaFlex vision encoder. The v28 v1.0 stack (perceptiond 8092, live since 2026-06-15) confirms the v17 choice. **No v28 swap.** LFM2.5-230M is the v28 v1.0 audiod post-processor (per v16). HRM-Text-1B is the v28 v1.5 plan-A post-processor (per v15). Apertus v1.1 4B is the v28 v1.5 plan-B. OpenPhone-3B is the v28 v1.5 plan-C. All v16 model decisions hold in v28.
> 2. **NEW HONORABLE MENTION #1 — Microsoft Research 300-paper survey as v28 *model-selector validation* (July 2026).** The survey confirms the v28 *evolutionary substrate* decomposition — weights / memory / tools — is the v28 canonical-published 2026 framing. **Concrete v28 read-through:** perceptiond = weights axis (LFM2.5-VL-450M), memoryd = memory axis (MiniLM-L6-v2), toold = tools axis (sandboxed shell). All three are v28 *canonically-survey-validated* as independent evolutionary substrates. This is the v28 *strongest possible architecture-validation* for the v17 9-daemon-substrate model choice.
> 3. **v28 confirms v17 whisper.cpp choice (held).** whisper-cpp-plus-rs async + VAD + real-time streaming is the v28 v1.0 STT. **No v28 swap.** v1.5 alternative: Parakeet for wake-word (VAD-only, faster wake).
> 4. **v28 confirms v17 KittenTTS choice (held).** KittenTTS base/medium is the v28 v1.0 TTS. **No v28 swap.** v28 honorable mention: Kokoro-82M is the v28 SOTA TTS benchmark (per v11), but KittenTTS is the v28 *right call* for v1.0 (CPU-friendly, ONNX, <25MB). Consider Kokoro-82M for v2.0 if voice quality becomes v1.0's #1 complaint.
> 5. **v28 confirms v25 MiniLM-L6-v2 for memoryd embeddings (held).** Per v27 MemDelta controlled-baseline protocol, swap-only-the-embedding-model shifts accuracy by +6.2pp. The v28 v1.0 v1.0 ships with MiniLM-L6-v2; v1.5 evaluates bge-small-en-v1.5 (smaller, 33MB vs 80MB) and mxbai-embed-large-v1 (SOTA but 335MB, too large for edge) in v28 *MemDelta-controlled* benchmarks. **v28 v1.5 plan-R2 (MHM-LRU four-head retention) is *architecture*, not model selection** — no embedding-model change is required.
> 6. **v28 retractions of v27:** **0.** All v27 model decisions hold. v28 adds v28 *substrate-validation* via the v28 Microsoft Research survey (memoryd = memory axis, toold = tools axis, perceptiond = weights axis) without changing any v28 model.
